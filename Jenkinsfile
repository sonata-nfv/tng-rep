    ...
    stage('Promoting release v5.0') {
        ...
        stages {
            stage('Generating release') {
                steps {
                    sh 'docker tag registry.sonata-nfv.eu:5000/tng-rep:latest registry.sonata-nfv.eu:5000/tng-rep:v5.0'
                    sh 'docker tag registry.sonata-nfv.eu:5000/tng-rep:latest sonatanfv/tng-rep:v5.0'
                    sh 'docker push registry.sonata-nfv.eu:5000/tng-rep:v5.0'
                    sh 'docker push sonatanfv/tng-rep:v5.0'
                }
            }
            stage('Deploying in v5.0 servers') {
                steps {
                    ...
                    sh 'git clone https://github.com/sonata-nfv/tng-devops.git'
                    dir(path: 'tng-devops') {
                    sh 'ansible-playbook roles/sp.yml -i environments -e "target=sta-sp-v5-0 component=repositories"'
                    sh 'ansible-playbook roles/vnv.yml -i environments -e "target=sta-vnv-v5-0 component=repositories"'
                    }
                }
            }
        }
